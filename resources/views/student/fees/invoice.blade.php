    ...
    <div class="tm_invoice_wrap">
        <div class="tm_invoice tm_style1" id="tm_download_section">
            <div class="tm_invoice_in">
                ...
                <!-- Installment Notice (if applicable) -->
                @if($invoice->is_installment)
                <div class="alert alert-info tm_mb20">
                    <h5 class="tm_mb2">INSTALLMENT PAYMENT</h5>
                    @if($invoice->payment)
                        <p class="tm_m0"><strong>Total Fee Amount:</strong> ₦{{ number_format($invoice->payment->amount, 2) }}</p>
                        <p class="tm_m0"><strong>This Installment:</strong> ₦{{ number_format($invoice->amount, 2) }}</p>
                        <p class="tm_m0"><strong>Installment Due Date:</strong> {{ $invoice->due_date->format('d F Y') }}</p>
                    @endif
                </div>
                @endif
                    ...
                    <div class="tm_round_border">
                        <div class="tm_table_responsive">
                            <table>
                                ...
                                <tbody>
                                    <tr class="tm_gray_bg">
                                        <td class="tm_width_5">{{ $invoice->paymentType->name }}
                                            @if($invoice->is_installment)
                                                <span class="text-xs">(Installment Payment)</span>
                                            @endif
                                        </td>
                                        <td class="tm_width_5">
                                            {{ optional($invoice->due_date ?? $invoice->created_at->addDays(7))->format('d F Y') }}
                        ...
                        <div class="tm_right_footer">
                            <table>
                                <tbody>
                                    ...
                                    <tr>
                                        <td colspan="2" class="tm_width_3 tm_text_right tm_border_none tm_pt0 tm_ternary_color">
                                            Please note: Additional transfer and processing charges may apply.
                                        </td>
                                    </tr>
                                    <tr class="tm_border_top">
                                    ...
                                    <tr class="tm_border_top">
                                        <td class="tm_width_3 tm_border_top_0 tm_bold tm_f16 tm_primary_color" colspan='2'>
                                            <form action="{{ route('student.fees.processPayment') }}" method="POST" class="d-inline">
                                                @csrf
                                                <input type="hidden" name="invoice_number" value="{{ $invoice->invoice_number }}">
                                                <input type="hidden" name="is_installment" value="{{ $invoice->is_installment }}">
                                                <input type="hidden" name="payment_type_id" value="{{ $invoice->payment_type_id }}">
                                                <input type="hidden" name="department_id" value="{{ $invoice->department_id }}">
                                                <input type="hidden" name="level" value="{{ $invoice->level }}">
                                                <input type="hidden" name="student_id" value="{{ $invoice->student_id }}">
                                                <input type="hidden" name="academic_session_id" value="{{ $invoice->academic_session_id }}">
                                                <input type="hidden" name="semester_id" value="{{ $invoice->semester_id }}">
                                                <input type="hidden" name="amount" value="{{ $invoice->amount }}">
                                                <input type="hidden" name="payment_method_id" value="{{ $invoice->payment_method_id }}">
                                                
                                                @if($invoice->status != 'paid')
                                                    <button onclick="return confirm('Are you sure you want to proceed with this {{ $invoice->is_installment ? 'installment ' : '' }}payment?')"
                                                            type="submit" class="btn btn-sm ml-3 no-print"
                                                            style="background:blueviolet;color:white">
                ...
                <div class="mt-3 tm_text_center tm_m0_md">
                    <p class="tm_m0">This invoice was created on a computer and is valid without the signature and seal.</p>
                </div>
            </div>
        </div>
        <div class="tm_invoice_btns tm_hide_print">
            ...
            <a href="{{ route('student.view.payments') }}" class="tm_invoice_btn tm_color1">
                <span class="tm_btn_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
                        <path d="M244 400L100 256l144-144M120 256h292" fill="none" stroke="currentColor"
                            stroke-linecap="round" stroke-linejoin="round" stroke-width="48" />
                ...
                <span class="tm_btn_text">Back to Payments</span>
            </a>
        </div>
    </div>
</div>
