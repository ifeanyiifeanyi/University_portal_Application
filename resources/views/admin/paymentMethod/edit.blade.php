...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-7 mx-auto">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.payment_method.update', $paymentMethod) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group mb-3">
                            <label for="name">Payment Method Name</label>
                            <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $paymentMethod->name) }}">
                            @error('name')
                        ...
                        <div class="form-group mb-3">
                            ...
                            @if($paymentMethod->logo)
                                <img src="{{ asset('storage/' . $paymentMethod->logo) }}" alt="Current Logo" class="img-fluid mt-2" style="max-width: 200px;">
                            @endif
                            @error('logo')
                                <p class="text-danger">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control" id="payment_type" name="config[payment_type]">
                                ...
                                <option value="credit_card" {{ old('config.payment_type', $paymentMethod->config['payment_type'] ?? '') == 'credit_card' ? 'selected' : '' }}>Credit Card</option>
                                <option value="bank_transfer" {{ old('config.payment_type', $paymentMethod->config['payment_type'] ?? '') == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
                                <option value="paypal" {{ old('config.payment_type', $paymentMethod->config['payment_type'] ?? '') == 'paypal' ? 'selected' : '' }}>PayPal</option>
                                <option value="cash" {{ old('config.payment_type', $paymentMethod->config['payment_type'] ?? '') == 'cash' ? 'selected' : '' }}>Cash</option>
                            </select>
                        ...
                        <div id="gatewaySelect" class="form-group mb-3">
                            ...
                            <select class="form-control" id="gateway" name="config[gateway]">
                                ...
                                <option value="paystack" {{ old('config.gateway', $paymentMethod->config['gateway'] ?? '') == 'paystack' ? 'selected' : '' }}>Paystack</option>
                                <option value="flutterwave" {{ old('config.gateway', $paymentMethod->config['gateway'] ?? '') == 'flutterwave' ? 'selected' : '' }}>Flutterwave</option>
                                <option value="remita" {{ old('config.gateway', $paymentMethod->config['gateway'] ?? '') == 'remita' ? 'selected' : '' }}>Remita</option>
                            </select>
        ...
        function updateConfigFields() {
            ...
            fields.forEach(field => {
                ...
                div.innerHTML = `
                    ...
                    <input type="${field.type}" class="form-control" id="config_${field.name}" name="config[${field.name}]" value="{{ old('config.${field.name}', $paymentMethod->config['${field.name}'] ?? '') }}" ${field.required ? 'required' : ''}>
                `;
