...
@section('admin')
                                ...
                                <select id="department" name="department_id" class="form-control" required>
                                    ...
                                    @foreach ($departments as $department)
                                        <option value="{{ $department->id }}"
                                            {{ $paymentType->departments->contains($department->id) ? 'selected' : '' }}>
                                            {{ $department->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <select id="payment_period" name="payment_period" class="form-control" required>
                                    ...
                                    <option {{ $paymentType->payment_period == 'semester' ? 'selected' : '' }}
                                        value="semester">Semester</option>

                                    <option {{ $paymentType->payment_period == 'session' ? 'selected' : '' }}
                                        value="session">Academic Session</option>
                                </select>
                                @error('semester_id')
                                    <p class="text-danger">{{ $message }}</p>
                                @enderror
                                ...
                                <select id="academic_session" name="academic_session_id" class="form-control" required>
                                    ...
                                    @foreach ($academicSessions as $session)
                                        <option value="{{ $session->id }}"
                                            {{ $paymentType->academic_session_id == $session->id ? 'selected' : '' }}>
                                            {{ $session->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select id="semester" name="semester_id" class="form-control" required>
                                    ...
                                    @foreach ($semesters as $semester)
                                        <option value="{{ $semester->id }}"
                                            {{ $paymentType->semester_id == $semester->id ? 'selected' : '' }}>
                                            {{ $semester->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <input type="number" class="form-control" id="late_fee_amount" name="late_fee_amount"
                                    value="{{ old('late_fee_amount', $paymentType->late_fee_amount) }}" required>
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <input type="number" class="form-control" id="grace_period_days" name="grace_period_days"
                                    value="{{ old('grace_period_days', $paymentType->grace_period_days) }}" required
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <input type="date" class="form-control" id="due_date" name="due_date"
                                    ...
                                    required placeholder="Late penalty fee">
                                @error('due_date')
                                    <p class="text-danger">{{ $message }}</p>
                                @enderror
                            </div>
 ...
 --}}
                    <div class="form-group mb-3">
                        ...
                        <div id="levelCheckboxes">
                            @php
                                $selectedDepartment = $departments->first(function ($dept) use ($paymentType) {
                                    return $paymentType->departments->contains($dept->id);
                                });
                                $selectedLevels = $paymentType->departments->pluck('pivot.level');
                            @endphp

                            @if ($selectedDepartment)
                                @foreach ($selectedDepartment->levels as $level)
                                    @php
                                        $numericLevel = $selectedDepartment->getLevelNumber($level);
                                    @endphp
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="levels[]"
                                            id="level{{ $numericLevel }}" value="{{ $numericLevel }}"
                                            {{ $selectedLevels->contains($numericLevel) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="level{{ $numericLevel }}">
                                            Level {{ $level }}
                                        </label>
                                    </div>
                                @endforeach
                    ...
                    <div class="form-group mb-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="is_active" name="is_active"
                                value="1" {{ $paymentType->is_active ? 'checked' : '' }}>
