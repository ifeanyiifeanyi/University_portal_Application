...
class StudentRecurringPaymentsController extends Controller
{
    ...
    public function calculatePayment(Request $request)
    {
        ...
        // Fetch the plan
        $plan = RecurringPaymentPlan::findOrFail($request->input('plan_id'));

        // Get selected months
        $selectedMonths = $request->input('selected_months');

        // Calculate base amount
        $baseAmount = count($selectedMonths) * $plan->amount;

        // Calculate late fees
        $lateFees = $this->calculateLateFees($selectedMonths);

        // Calculate total late fees
        $totalLateFees = array_sum($lateFees);

        // Calculate total amount
        $totalAmount = $baseAmount + $totalLateFees;
        ...
        return response()->json([
            ...
            'late_fee_per_month' => env('FEEDING_FEE_LATE')
        ]);
    }

    private function calculateLateFees(array $selectedMonths)
    {
        $lateFees = [];
        $currentYear = date('Y');
        $lateFeeAmount = env('FEEDING_FEE_LATE');

        foreach ($selectedMonths as $month) {

            $deadline = Carbon::create($currentYear, $month, 6);


            if (Carbon::now()->gt($deadline)) {
                $lateFees[$month] = $lateFeeAmount;
            } else {
                $lateFees[$month] = 0;
            }
        }
    ...
    public function handleCallback(Request $request)
    {
        ...
        try {
            ...
            if (!$found || !$subscription) {
                ...
                if (
                    isset($paystackResult['status']) && $paystackResult['status'] &&
